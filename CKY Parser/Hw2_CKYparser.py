...
###Question 1: CKY parser 

import copy
import sys
import nltk

###Initially CFG is inputted from the grammar file and this is converted to CNF by the code and written to the same grammar file
###After this CNF Grammar produced is cross-checked with one manually produced
###The CKY algorithm is run on the CNF Grammar hereafter and the output is produced
...
class Grammar:
    ...
    def writeToFile(self, outputFile):
        ...
        grammarFile.write('\t CNF Grammar for the sentence' + '\n\n')
...
class Parser:
    ...
    def writeParsesToFile(self, fileName):
        ...
        for sentence in self._sentences:
            ...
            output.write('\n\nNumber of parses for the sentence: ' + str(parseCnt) + '\n\n')
...
cnfG.writeToFile(grammarFile)

#   load sentences to parse
p = CkyParser(cnfG)
p.load(sentenceFile)
#   print parses to file
p.writeParsesToFile(outputFile)